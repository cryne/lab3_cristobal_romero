	...
	while(!estado){
		...
		for (int i = 0; i < 4; ++i)
		{
			...
			if(x>-1&&x<6&&y>-1&&y<7){
				...
				if(tablero[x-1][y+1]==0||tablero[x-1][y+1]==2){
					i=5;
					int x1=x,y1=y;
					victoria=0;
					for (int j = 0; j < 4; ++j)
		...
		if(victoria!=4){
			...
			for (int i = 0; i < 4; ++i)
			{
				int x=jugada1,y=jugada-i;	
				if(x>-1&&x<6&&y>-1&&y<7){
					if(tablero[x][y]==1){
						++victoria;
					}
					if(tablero[x][y-1]==0||tablero[x][y-1]==2){
						i=5;
						int x1=x,y1=y;
						victoria=0;
						for (int j = 0; j < 4; ++j)
		...
		if(!estado){
			...
			int jugada=-1;
			while(jugada<0||jugada>6){
				cout<<"jugador 2 ingrese la casilla en la que desea lanzar su pieza"<<endl;
				cin>>jugada;
				if (jugada<0||jugada>6)
				{
					cout<<"jugada no valida"<<endl;
			...
			for (int i = 0; i < 4; ++i)
			{
				...
				if(x>-1&&x<6&&y>-1&&y<7){
					...
					if(tablero[x-1][y+1]==0||tablero[x-1][y+1]==1){
						i=5;
						int x1=x,y1=y;
						victoria=0;
						for (int j = 0; j < 4; ++j)
			...
			if(victoria!=4){
				...
				for (int i = 0; i < 4; ++i)
				{
					...
					if(x>-1&&x<6&&y>-1&&y<7){
						...
						if(tablero[x][y-1]==0||tablero[x][y-1]==1){
							i=5;
							int x1=x,y1=y;
							victoria=0;
							for (int j = 0; j < 4; ++j)
